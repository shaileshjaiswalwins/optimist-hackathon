...
export const MiniMap = ({ myPlayerId, profiles, positions, vitals, fieldSize }: MiniMapProps) => {
  // fieldSize should be the actual racer count, not the lobby's static
  // max-slots cap — the server's own lane math keys off live field size too.
  const halfWidth = roadHalfWidthFor(fieldSize || 1);
  ...
  return (
    <div className="jaldi-minimap">
      <style>{`
        .jaldi-minimap { position:fixed; z-index:15; top:max(84px, calc(env(safe-area-inset-top) + 68px)); left:max(12px, env(safe-area-inset-left)); width:${WIDTH}px; height:${HEIGHT}px; background:rgba(8,15,10,.62); backdrop-filter:blur(10px); border-radius:10px; border:1px solid rgba(255,255,255,.12); overflow:hidden; transform-origin:top left; }
        ...
        /* Scale the whole panel down on phones instead of recomputing dot
           positions against a second set of dimensions, so dots never clip. */
        @media (max-width:640px) { .jaldi-minimap { transform:scale(0.74); top:max(64px, calc(env(safe-area-inset-top) + 50px)); } }
      `}</style>
